...
def join_binding_regions(clip_regular_counts, sm_counts,
                         joined_gtf_file):
    """Takes input BED files and joins all regions into a dictionary,
        removing all duplicate/overlapping regions.

        Arguments:
        clip_regular_counts -- BED file produced from Piranha or other CLIP
            read pileup tool.
        sm_counts -- List of additional BED files (strings) 
    ...
    region_dictionary = add_events_to_dictionary(clip_regular_counts)
...
def add_events_to_dictionary(clip_regular_counts):
    ...
    with open(clip_regular_counts) as open_bedfile:
        for line in open_bedfile:
            # Replaces tabs with underscore. This makes it so the literal tab
...
def init_argparse():
    ...
    parser.add_argument("-b1", "--clip_regular_counts", action = "store", type = str, 
                        help="BED file produced from Piranha or other CLIP read pileup tool.",
                        required = True)
    parser.add_argument("-b2", "--sm_counts", action = "store", type = str, nargs='+', 
    ...
    parser.add_argument("-b2", "--sm_counts", action = "store", type = str, nargs='+', 
                        ...
                        each BED file needed to be joined separated by a space.", 
                        required = True)
    parser.add_argument("-o", "--filtered_annotated_counts", action = "store", type = str, 
...
def main():
    """Main script function. Executes other functions.
        ...
        Arguments:
        clip_regular_counts -- DeSeq2 produced counts file for a single protein
            concentration (produced with replicates). This 
        sm_counts -- Additional BED files needed to join first BED file.
            BED files should be produced from Piranha or other CLIP read pileup tool.
            This is a list of one or more BED files.
        regular_gtf_file -- Output GTF file. Contains nonrepetitive, comprehensive
            read pileup regions from each input sample.    
        Output:    
        filtered_annotated_counts -- Output GTF file. Contains nonrepetitive, comprehensive
            read pileup regions from each input sample.
    """
    ...
    join_binding_regions(parsed.clip_regular_counts, parsed.sm_counts,
                         parsed.regular_gtf_file, parsed.filtered_annotated_counts)
